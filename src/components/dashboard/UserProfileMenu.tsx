      ...
      <Card className="absolute bottom-16 left-4 w-64 shadow-xl z-[100] p-0 overflow-hidden border-2 border-wastewise-light-green/20 bg-white animate-fade-in">
        <div className="p-4 border-b flex items-center space-x-3 bg-wastewise-light-beige">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-wastewise-gray truncate text-left">{user?.email}</p>
          </div>
        </div>
        
        <div className="py-2">
          <Button variant="ghost" className="w-full justify-start px-4 py-2 h-auto hover:bg-wastewise-light-green/10" onClick={() => handleNavigation('/dashboard/profile')}>
            <User className="h-4 w-4 mr-3" />
            <span className="text-sm">View Profile</span>
          </Button>
          <Button variant="ghost" className="w-full justify-start px-4 py-2 h-auto hover:bg-wastewise-light-green/10" onClick={() => handleNavigation('/dashboard/settings')}>
            <Settings className="h-4 w-4 mr-3" />
            <span className="text-sm">Settings</span>
          </Button>
          <Button variant="ghost" className="w-full justify-start px-4 py-2 h-auto hover:bg-wastewise-light-green/10" onClick={() => handleNavigation('/dashboard/subscription')}>
            <CreditCard className="h-4 w-4 mr-3" />
            <span className="text-sm">Subscription</span>
          </Button>
          <Button variant="ghost" className="w-full justify-start px-4 py-2 h-auto hover:bg-wastewise-light-green/10">
